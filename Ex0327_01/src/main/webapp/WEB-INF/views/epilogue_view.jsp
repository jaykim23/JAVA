...
<script type="text/javascript">
	function delete_check(){
		if(confirm('정말로 삭제하시겠습니까?')){
			location.href = "delete.do?category=${category}&search=${search}&page=${page}&bId=${dto.bId}";
			return true;
		}else{
			return false;
		}
				...
				<div id="outbox">
					...
					<form action="">
						...
						<div id="contents">
							<div id="mypage">
								<h2>
									<strong>게시판</strong><span>게시판의 글입니다.</span>
								</h2>
								...
								<div class="viewDivTab">
									<div class="viewHead">
										<div class="subject">
											<ul>
												<li class="cate">[게시글${dto.bId}]</li>
												<li>&nbsp;${dto.bTitle}</li>
											</ul>
										</div>
										<div class="day">
											<p class="txt">
												파일명<span>${dto.fileName} </span>
											</p>
										</div>
										<div class="data">
											<ul>
												<li>작성자<span>${dto.bName}</span></li>
												<li class="tnone">등록일<span><fmt:formatDate
															value="${dto.bDate}" pattern="yyyy/MM/dd" /></span></li>
												<li class="tnone">조회수<span>${dto.bHit}</span></li>
												<li class="last">평점 <span> <img
...
${dto.bContent}"
										...
										<c:choose>
											<c:when test="${dto.fileName == null}"></c:when>
											<c:otherwise>
												<img src="upload/${dto.fileName}"width="300px" height="200px"/>
											</c:otherwise>
										</c:choose>
									</div>
								</div>
								...
								<!--  <div class="answer">
									<div class="inbox">
										<div class="aname">
											<p>
												${dto.bName} <span>${dto.bDate}</span>
											</p>
										</div>

										<div class="atxt">
											${dto.bContent}
											<img src="upload/${dto.fileName}"width="120" height="95"/>
										</div>
									</div>
								</div>-->
								<!-- //답변 -->
								...
								<div class="btnArea">
									<div class="bRight">
										<ul>
											<li><a onclick="return delete_check()"class="nbtnbig mw">삭제</a></li>
											<li><a
												href="modify_view.do?category=${category}&search=${search}&page=${page}&bId=${dto.bId}"
												class="nbtnbig mw">수정</a></li>
											<li><a
												href="reply_view.do?category=${category}&search=${search}&page=${page}&bId=${dto.bId}"
												class="nbtnbig mw">답글</a></li>
											<li><a
												href="list.do?category=${category}&search=${search}&page=${nowpage}"

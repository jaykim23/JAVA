...
<div id="wrap">
	...
	<div id="container">
		...
		<div id="outbox">		
			...
			<div id="contents">
				<div id="mypage">
					<h2><strong>게시판</strong><span>쟈뎅 제품을 구매하신 회원님들의 소중한 후기입니다.</span></h2>
							...
							<li>
								<c:choose>
									<c:when test="${dto.fileName == null}">
										<div class="img"><img alt="" src="upload/noimage.gif" width="155" height="160"></div>
									</c:when>
									<c:otherwise>
										<div class="img"><img src="upload/${dto.fileName}" width="155" height="160"/></div>
									</c:otherwise>
								</c:choose>
								<div class="txt">
									<div class="subject">
										<a href="content_view.do?category=${category}&search=${search}&page=${page}&bId=${dto.bId}"><span class="orange">[게시글${dto.bId}]</span>${dto.bTitle}</a>
									</div>
									<div class="conf" style="width:100%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap">
										${dto.bContent}
									</div>
									<div class="data">
										<p>작성자 <span>${dto.bName}</span></p>
										<p>등록일 <span><fmt:formatDate value="${dto.bDate}" pattern="yyyy/MM/dd"/></span></p>
										<p>조회수 <span>${dto.bHit}</span></p>
										<p>평점 
											<span class="ty">
					...
					<div class="btnAreaList">
						<div class="bwright">
							<ul>
								<li><a href="write_view.do" class="sbtnMini">글쓰기</a></li>
							</ul>
						</div>
						...
						<a href="list.do?category=${category}&search=${search}&page=${startpage}" class="n"><img src="images/btn/btn_pre2.gif" alt="처음으로"/></a>
						<a href="list.do?category=${category}&search=${search}&page=${page-1}" class="pre"><img src="images/btn/btn_pre1.gif" alt="앞페이지로"/></a>
						<c:forEach var="nowpage" begin="${startpage}" end="${endpage}">
							<c:choose>
								<c:when test="${page==nowpage }">
									<a><strong>${nowpage}</strong></a>
								</c:when>
								<c:otherwise>
									<a href="list.do?category=${category}&search=${search}&page=${nowpage}">${nowpage}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<a href="list.do?category=${category}&search=${search}&page=${page+1}" class="next"><img src="images/btn/btn_next1.gif" alt="뒤페이지로"/></a>
						<a href="list.do?category=${category}&search=${search}&page=${maxpage}" class="n"><img src="images/btn/btn_next2.gif" alt="마지막페이지로"/></a>
